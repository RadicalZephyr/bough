...
fn the_build_closure_runs_as_transaction_zero_and_returns_its_edge() {
    ...
    let (_numbers_in, _limit, _never) = edge.keep();
...
fn a_coalescing_input_accepts_several_sends_in_one_transaction() {
    let (mut graph, edge) = Runtime::build(|b| b.input_coalescing(|x: u32, y| x + y).1);
    let a_in = edge.keep();
...
fn a_token_from_another_graph_is_an_error_and_leaves_the_graph_usable() {
    let (mut first, edge) = Runtime::build(|b| b.input::<u32>().1);
    let first_in = edge.keep();
    let (mut second, edge) = Runtime::build(|b| b.input::<u32>().1);
    let second_in = edge.keep();
...
fn a_foreign_token_in_graph_code_panics() {
    let (_other, edge) = Runtime::build(|b| b.constant(1u32));
    let stranger = edge.keep();
